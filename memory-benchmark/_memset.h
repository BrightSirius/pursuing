...
void memset_mp(void* bytes, int size, unsigned char val){
    int i, bid;
    int blk_cnt=(1<<10);
...
void fast_memset128(void* bytes, int size, unsigned char val){
    int step = 16;
    assert(size % step == 0);
...
void fast_memset256(void* bytes, int size, unsigned char val){
    int step = 32;
    assert(size % step == 0);
    assert((long long)(bytes) % step == 0);
...
void fast_memset256_mp(void* bytes, int size, unsigned char val){
    int step = 32;
    assert(size % step == 0);
    assert((long long)(bytes) % step == 0);
    __m256i vals = _mm256_set1_epi8(val);
